$(function () {
    ...
    /*    $('.faq_t li').on('click',function(){
            $(this).siblings().find('.ans').stop().slideUp(600);
            $(this).find('.anw').stop().slideDown(600);
        });,function(){
            $(this).find('.anw').stop().slideUp(600);
        });
        */

    /*   var list=$('.faq_t ul>li');
	list.click(function(){
		$(this).siblings().find('.anw').stop().slideUp(250);
		$(this).children('.anw').stop().slideDown(250);
	}*/
    /*,function(){
    		$(this).find('.anw').stop().slideUp(250);
    	});*/
    ...
    list.on('click', function (event) {
        ...
        if (option.css('display') == 'none') {
            list2.siblings().find('.anw').stop().slideUp(250);
            option.stop().slideDown(250);

        } else {
            option.stop().slideUp(250);
        }
    });
});
